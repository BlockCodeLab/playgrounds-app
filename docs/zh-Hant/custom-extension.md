...
在使用離線版本的編輯器時，可以通過添加自製擴展來滿足用戶在項目中使用了編輯器暫未支持傳感器的編程需要。這些本地的自製擴展會被保存到作品中，在其他電腦上同樣用離線版本打開時會被一同加載，保證使用自製擴展的作品能在任何電腦上打開。

!> 使用了自製擴展的作品無法被在線版本的編輯器打開，在線版本只能使用編輯器本身支持的擴展編程。

## 使用自製擴展

自製擴展保存的位置（如果文件夾不存在請自行創建）：

- Windows：`C:\Users\<用戶名>\BlockCode\blocks\`
- macOS：`/Users/<用戶名>\BlockCode\blocks\`

每個自製擴展一個文件夾，文件夾名也是自製擴展ID，請確保每個自製擴展的文件夾名唯一且只能由字母、數字和下劃線。

!> 確定自製擴展ID後，請不要更改，更改有可能導致原來用到該自製擴展的作品無法打開。

### 添加自製擴展

在離線版本的編輯器中，進入「添加擴展」界面，在擴展分類標籤最後會有「自製」擴展的標籤，點擊標籤後會展示出所有已經保存在本地的自製擴展。點擊「+」圖標的占位符，可以添加自製擴展（支持添加使用 ZIP 壓縮包的壓縮的一個或多個擴展）。

![](_media/custom-extension.png ':size=600')

> [下載案例](_res/dust.zip ':ignore')直接添加嘗試。

也可以直接在自製擴展的文件夾中手動添加自製擴展，詳細方法見[自製擴展案例](#自製擴展案例)。

### 保存使用自製擴展的作品

只需要按照正常保存作品的流程將作品「保存到電腦」，保存的作品中就會自帶所使用的自製擴展。

![](_media/save-computer.png ':size=300')

### 導出自製擴展

想要導出一個自己在使用的自製擴展給其他人使用，可以通過[保存使用自製擴展的作品](#保存使用自製擴展的作品)發給其他人（最為簡單）,在其他電腦上打開一次作品後就會自動添加使用的自製擴展。

也可以直接進入自製擴展的文件夾，選中要導出的自製擴展（一個或多個），使用壓縮工具壓縮成 ZIP 壓縮包就可以了。其他人只需要按正常[添加自製擴展](#添加自製擴展)的方式就可以添加。

## 自製擴展案例

下面通過一個簡單的案例來了解如何創建一個自製擴展。
...
!> `info.js` 和 `index.js` 必須放在自製擴展文件夾的首層路徑。
