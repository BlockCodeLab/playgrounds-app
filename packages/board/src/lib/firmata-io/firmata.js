...
const SYSEX_RESPONSE = {
  ...
  [PING_READ](board) {
    ...
    const duration =
      (durationBuffer[0] << 24) + (durationBuffer[1] << 16) + (durationBuffer[2] << 8) + durationBuffer[3];
  ...
  [ACCELSTEPPER](board) {
    ...
    const value = command === 0x06 || command === 0x0a ? decode32BitSignedInteger(board.buffer.slice(4, 9)) : null;
...
class Firmata extends EventEmitter {
  ...
  getRUS04(pin, callback) {
    ...
    writeToTransport(this, [START_SYSEX, RUS04, pin, END_SYSEX]);
  }

  getDHTTemp(pin, callback) {
    this.once('get-dht-temp', callback);
    writeToTransport(this, [START_SYSEX, DHT_TEMP, pin, END_SYSEX]);
  }

  getDHTHum(pin, callback) {
    this.once('get-dht-hum', callback);
    writeToTransport(this, [START_SYSEX, DHT_HUM, pin, END_SYSEX]);
  }
  ...
  i2cRead(address, register, bytesToRead, callback) {
    ...
    const data = [START_SYSEX, I2C_REQUEST, address, this.I2C_MODES.CONTINUOUS_READ << 3];
  ...
  [SYM_sendOneWireRequest](
    ...
    const output = [START_SYSEX, ONEWIRE_DATA, subcommand, pin, ...Encoder7Bit.to7BitArray(bytes), END_SYSEX];
  ...
  _play_tone(pin, tone_command, frequency, duration) {
    ...
    if (tone_command == TONE_TONE) {
      if (duration) {
        ...
        data = [START_SYSEX, TONE_DATA, tone_command, pin, frequency & 0x7f, (frequency >> 7) & 0x7f, 0, 0, END_SYSEX];
      }
    } else {
  ...
  pingRead(options, callback) {
    ...
    writeToTransport(this, [
      ...
      pin,
      value,
      ...Firmata.encode([(pulseOut >> 24) & 0xff, (pulseOut >> 16) & 0xff, (pulseOut >> 8) & 0xff, pulseOut & 0xff]),
      ...Firmata.encode([(timeout >> 24) & 0xff, (timeout >> 16) & 0xff, (timeout >> 8) & 0xff, timeout & 0xff]),
      END_SYSEX,
    ]);
  ...
  stepperStep(deviceNum, direction, steps, speed, accel, decel, callback) {
    ...
    writeToTransport(this, [
      ...
      ...(accel > 0 || decel > 0 ? [accel & 0x7f, (accel >> 7) & 0x7f, decel & 0x7f, (decel >> 7) & 0x7f] : []),
  ...
  serialRead(portId, maxBytesToRead, callback) {
    const data = [START_SYSEX, SERIAL_MESSAGE, SERIAL_READ | portId, this.SERIAL_MODES.CONTINUOUS_READ];
...
function encode32BitSignedInteger(data) {
  ...
  const encoded = [data & 0x7f, (data >> 7) & 0x7f, (data >> 14) & 0x7f, (data >> 21) & 0x7f, (data >> 28) & 0x07];
